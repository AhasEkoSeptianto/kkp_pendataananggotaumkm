    ...
    <div className="bg-blue-secondary min-h-screen relative shadow-lg hidden lg:block col-span-2">
      <div className="py-10 flex flex-col items-center justify-center">
        <div className="w-24 h-24 rounded-full overflow-hidden">
          <Image preview={false} src={users?.profile_picture} width={200} />
          {/* <Avatar src={users?.profile_picture} /> */}
        </div>
      ...
      <div>
        {ListMenu?.map((item, idx) => (
          <Link href={item?.link} key={idx}>
            <div
              className={`py-3 px-5 cursor-pointer flex items-center space-x-4 ${
                router.pathname === item?.link
                  ? "bg-blue-darkSecondary"
                  : "hover:bg-blue-darkSecondary"
              }`}
            >
              {item?.icon}
...
export function SidebarMobile() {
  ...
  return (
    <Fragment>
      <div
        className="absolute w-screen h-screen bg-gray-800 bg-opacity-80 flex flex-col items-center justify-center"
        style={{ zIndex: 1400 }}
      >
        {ListMenu?.map((item, idx) => (
          <Link href={item?.link} key={idx}>
